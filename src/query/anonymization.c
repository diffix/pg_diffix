...
static void expand_implicit_buckets(Query *query)
{
  ...
  count_agg->aggfnoid = g_oid_cache.count_row; /* Will be replaced later with anonymizing version. */
...
static Node *aggregate_expression_mutator(Node *node, QueryContext *context)
{
  ...
  else if (IsA(node, Aggref))
  {
    ...
    if (aggfnoid == g_oid_cache.count_row)
      rewrite_to_anon_aggregator(aggref, context, g_oid_cache.anon_count_row);
